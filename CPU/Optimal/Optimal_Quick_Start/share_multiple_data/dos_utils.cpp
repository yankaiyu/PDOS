...
unordered_map<int, vector<int> > *raw_data_maps;
/*
 * Class UserTrace
 */
...
void ResultsPerUser::deepenOneLevel() {
    ...
    for (int i = 0; i < previous_level_user_count; i++) {
        ...
        vector<int> friend_list_of_user = raw_data_maps[thread_num].find(user_id)->second;
...
void ResultsAllUsers::initThreadRawData() {
    int num_threads = omp_get_max_threads();
    raw_data_maps = new unordered_map<int, vector<int> >[num_threads];
    for (int i = 0; i < num_threads; i++) {
        raw_data_maps[i] = *(DataUtils::getInstance()->getRawDataMap());
    }
